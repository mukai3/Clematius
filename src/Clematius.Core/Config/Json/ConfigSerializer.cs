...
namespace Clematius.Core.Config.Json;

/// <summary>
/// ClematiusConfig の JSON 入出力。アクションの判別共用体変換を含む。
/// </summary>
public static class ConfigSerializer
{
    ...
    public static string Serialize(ClematiusConfig config)
        => JsonSerializer.Serialize(config, Options);

    public static ClematiusConfig Deserialize(string json)
    {
        var config = JsonSerializer.Deserialize<ClematiusConfig>(json, Options);
        if (config is null)
    ...
    /// </summary>
    private static ClematiusConfig MigrateLegacyWheel(ClematiusConfig config, string json)
    {
