...
internal sealed class ConfigStore : IDisposable
{
    ...
    public event Action<ClematiusConfig>? Changed;
    ...
    /// <summary>
    /// 旧名 "Clemoutis" 時代の設定を一度だけ引き継ぐ（Clemoutis→Clematius リネームの移行措置）。
    ...
    private ClematiusConfig LoadOrCreate()
    {
    ...
    private bool TryLoad(out ClematiusConfig? config, out string? backupPath)
    {
        config = null;
        backupPath = null;
        try
    ...
    public void Save(ClematiusConfig config)
    {
