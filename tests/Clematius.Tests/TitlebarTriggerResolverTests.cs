...
using Clematius.Core.Windowing;
...
public class TitlebarTriggerResolverTests
{
    ...
    {
        var config = new Clematius.Core.Config.ClematiusConfig
        {
            Titlebar = Settings with { WindowOpacity = 70 },
        };
        var restored = Clematius.Core.Config.Json.ConfigSerializer.Deserialize(
            Clematius.Core.Config.Json.ConfigSerializer.Serialize(config));
